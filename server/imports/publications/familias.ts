...
Meteor.methods({
  ...
  updateFamilia(usuario: Familia){
    Familias.update({ _id: usuario._id }, {$set:{ nombre: usuario.nombre, apellidos: usuario.apellidos, email: usuario.email, comienzo: usuario.comienzo, miembros: usuario.miembros, dificultad: usuario.dificultad, tiempo: usuario.tiempo, gustos_nutrientes: usuario.gustos_nutrientes }});
  },
